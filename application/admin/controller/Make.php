...
class Make extends Base
{
    ...
    public function rss()
    {
        ...
        $this->label_maccms();
        $link = mac_url('rss/'.$this->_param['ac2']);
    ...
    public function type()
    {
        ...
        if(ENTRANCE=='api'){
            mac_echo('定时任务完毕，本次生成'.$GLOBALS['config']['app']['makesize'].'个列表页面，避免网站卡死！');
            die;
        }
    ...
    public function info()
    {
        ...
        if($start > $page_count){
            ...
            if($this->_param['ac2'] =='nomake' || $this->_param['data_count']==0 ){
                mac_jump( url('make/opt') ,3);
                die;
            }
            else{
        ...
        foreach($res['list'] as $k=>$v){
            ...
            if(!empty($v['art_id'])) {
                ...
                $flag = ['play','down'];
                foreach($flag as $f) {
                    $GLOBALS['aid'] = mac_get_aid('vod',$flag);
                    $this->label_maccms();
                    //播放页 和 下载页
