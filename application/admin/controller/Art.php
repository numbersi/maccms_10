    ...
    public function data()
    {
        ...
        if(!empty($param['repeat'])){
            if($param['page'] ==1){
                ...
                Db::query('CREATE TABLE IF NOT EXISTS `'.config('database.prefix').'tmpart` as (SELECT art_name as name1 FROM '.config('database.prefix').'art GROUP BY name1 HAVING COUNT(name1)>1)');
            }
            $order='art_name asc';
