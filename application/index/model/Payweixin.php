...
class Payweixin extends Base {
    ...
    public function submit($user,$order,$param)
    {
        ...
        $data['spbill_create_ip'] =  (int)bindec(decbin( ip2long(request()->ip() ) ));//终端IP
        $data['notify_url'] =  'http://' . $_SERVER['SERVER_NAME'] . '/index.php/payment/notify/pay_type/weixin';
    ...
    public function notify()
    {
        ...
        unset($data['sign']);
        // 生成签名
        $sign = $this->makeSign($config,$data);
        // 判断签名是否正确  判断支付状态
