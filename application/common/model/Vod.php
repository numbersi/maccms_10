...
class Vod extends Base {
    ...
    public function listData($where,$order,$page=1,$limit=20,$start=0,$field='*',$addition=1,$totalshow=1)
    {
        ...
        foreach($list as $k=>$v){
            $list[$k]['vod_pic'] = $this->image($v['vod_pic']);
            if($addition==1){
                if(!empty($v['type_id'])) {
    ...
    public function listCacheData($lp)
    {
        ...
        if(!empty($rel)){
            $tmp = explode(',',$rel);
            if(is_numeric($rel) || count($tmp)>1 ){
                $where['vod_id'] = ['in',$tmp];
            }
            else{
                $where['vod_rel_vod'] = ['eq',$rel];
            }
        }
        if(!empty($level)) {
            if($level=='all'){
        ...
        if(!empty($wd)) {
            $where['vod_name|vod_en|vod_sub|vod_actor'] = ['like', '%' . $wd . '%'];
        }
        if(!empty($tag)) {
        ...
        if($by=='rnd'){
            $datacount = mac_data_count(0,'all','vod');
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        $info['vod_pic'] = $this->image($info['vod_pic']);
    ...
    protected function image($imges)
    {



        $imges =str_replace("tu.php?tu=","http://", $imges);
        if(strpos($imges,'qiyipic.com') !== false){
            $imges = 'index.php/index/image?url='.$imges;
        }
