...
//            $url = $param['cjurl']."?wd=".$param['wd'];
            $html = $this->getVodByWd($url);
        }
        var_dump($url);
        if(empty($html)){
    ...
    public function vod_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif(strpos(','.$config['filter'],$v['vod_name'])!==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['vod_id']);
                ...
                if (!$info) {
                    ...
                    if ($info['vod_lock'] == 1) {
                        ...
                        if (strpos(',' . $config['uprule'], 'j')!==false && substr($info["vod_pic"], 0, 4) == "http" && $v['vod_pic']!=$info['vod_pic'] ) {
                            $tmp = $this->syncImages($config['pic'],$v['vod_pic'],'vod');
    ...
    public function art_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif(strpos(','.$config['filter'],$v['art_name'])!==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['art_id']);
                ...
                if (!$info) {
                    ...
                    if ($info['art_lock'] == 1) {
                        ...
                        if($rc){
                            ...
                            if(strpos(','.$config['uprule'],'b')!==false && !empty($v['act_author']) && $v['act_author']!=$info['act_author']){
                            ...
                            if(strpos(','.$config['uprule'],'d')!==false && substr($info["art_pic"], 0, 4) == "http" && $v['art_pic']!=$info['art_pic'] ){
                                $tmp = $this->syncImages($config['pic'],$v['art_pic'],'art');
    ...
    public function actor_data($param,$data,$show=1)
    {
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif(strpos(','.$config['filter'],$v['actor_name'])!==false) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['actor_id']);
                ...
                if (!$info) {
                    ...
                    if ($info['actor_lock'] == 1) {
                        ...
                        if($rc){
                            ...
                            if(strpos(','.$config['uprule'],'e')!==false && substr($info["actor_pic"], 0, 4) == "http" && $v['actor_pic']!=$info['actor_pic'] ){
                                $tmp = $this->syncImages($config['pic'],$v['actor_pic'],'actor');
    ...
    protected function getVodByWd($url )
    {
        $html = mac_curl_get($url);
