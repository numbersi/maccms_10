...
class User extends Base
{
    ...
    public function register($param)
    {
        ...
        if (empty($param['user_openid_qq']) && empty($param['user_openid_weixin']) && !captcha_check($data['verify'])) {
    ...
    public function login($param)
    {
        ...
        $update['user_login_ip'] = (int)bindec(decbin( ip2long(request()->ip() ) ));
    ...
    public function promoter($data)
    {
        ...
        $where2['visit_ip'] = (int)bindec(decbin( ip2long(request()->ip() ) ));
    ...
    {
        $param['user_email'] = htmlspecialchars(urldecode(trim($param['user_email'])));

        if(!in_array($param['ac'],['email']) || empty($param['user_email'])){
            return ['code'=>2001,'msg'=>'参数错误'];
        }
        $to = $param['user_email'];
        $pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
        ...
        $where=[];
        $where['user_email'] = $param['user_email'];
        $user = $this->where($where)->find();
        if(!$user){
            return ['code'=>2003,'msg'=>'邮箱地址错误'];
        }

        $code = mac_get_rndstr(6,'num');
        $title = '【'.$GLOBALS['config']['site']['site_name'].'】找回密码验证';
        $msg = '【'.$GLOBALS['config']['site']['site_name'].'】的会员您好，'.$GLOBALS['user']['user_name']."<br>".'找回密码验证码为：'. $code .',请在5分钟内完整验证。' ;
    ...
    public function findpass_reset($param)
    {
        ...
        if(!in_array($param['ac'],['email']) || empty($param['user_email']) || empty($param['user_pwd']) || empty($param['user_pwd2']) ){
            return ['code'=>2001,'msg'=>'参数错误'];
        }
        $to = $param['user_email'];
        $pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
        ...
        if (strlen($param['user_pwd']) <6) {
            return ['code' => 2003, 'msg' => '密码最少6个字符'];
        }
        if ($param['user_pwd'] != $param['user_pwd2']) {
            return ['code' => 2004, 'msg' => '密码与确认密码不一致'];
        }
        ...
        $res = model('msg')->infoData($where);
        if($res['code'] >1){
            return ['code'=>2005,'msg'=>'验证信息错误，请重试'];
        }
        ...
        $res = $this->where($where)->update($update);
        if($res===false){
            return ['code'=>2007,'msg'=>'修改免失败，请重试'];
        }
